...
class fitOscParams(object):
    ...
    def llhFunction(self,dm31 = None,  
			 ...
			 nu_frac9 = None,
                    ...
                    #now nue  (8 bins), numu was 9 bins
                         nu_frac10 = None,
                         nu_frac11 = None,
                         nu_frac12 = None,
	...
	print 'neutrino histrogram 0 sum, neutrino histogram 1 sum'
	print np.sum(neutrinos[0]), np.sum(neutrinos[1]) , np.sum(neutrinos[2]), np.sum(neutrinos[3]), np.sum(neutrinos[4]), np.sum(neutrinos[5]), np.sum(neutrinos[6]), np.sum(neutrinos[7]), np.sum(neutrinos[8]), np.sum(neutrinos[9])
        ...
        norm_9 = nu_events*nu_frac9/(np.sum(neutrinos[8])*norm_nu)
         #nue fractions
	norm_10 = nu_events*nu_frac10/(np.sum(neutrinos[9])*norm_nu)
        norm_11 = nu_events*nu_frac11/(np.sum(neutrinos[10])*norm_nu)
        norm_12 = nu_events*nu_frac12/(np.sum(neutrinos[11])*norm_nu)
	norm_13 = nu_events*(1-nu_frac1-nu_frac2-nu_frac3-nu_frac4-nu_frac5-nu_frac6-nu_frac7-nu_frac8-
                             nu_frac9-nu_frac10-nu_frac11-nu_frac12)/(np.sum(neutrinos[12])*norm_nu)
        if norm_13 < 0:
            print 'oscFit: Norm 9 , or 17 as we now call it, was calculated to be negative! using absolute value'
            norm_13 = np.abs(norm_13)
	...
	print '                                                 '
        nu_norms = [norm_1, norm_2, norm_3, norm_4,norm_5, norm_6, norm_7, norm_8, norm_9,norm_10, norm_11, norm_12, norm_13, 0.]
	print 'nu norms as we go, ie norm 1 = Nu_events*original_nu_Frac1/(sum (first neurtrino histogram) * nurm_nu )'
	print nu_norms
	print '                                                    '
	print 'nu_frac1,nu_frac2,nu_frac3,nu_frac4,nu_frac5,nu_frac6,nu_frac7,nu_frac8,nu_frac9,nu_frac10,nu_frac11,nu_frac12, nu_frac13' 
	print nu_frac1,nu_frac2,nu_frac3,nu_frac4,nu_frac5,nu_frac6,nu_frac7,nu_frac8,nu_frac9,nu_frac10,nu_frac11,nu_frac12,(1-nu_frac1-nu_frac2-nu_frac3-nu_frac4-nu_frac5-nu_frac6-nu_frac7-nu_frac8-nu_frac9-nu_frac10-nu_frac11-nu_frac12)
	print '                                                   '
        norm_atmmu = 0.
        norm_noise = 0.
        if atmmu_f != 0:
        ...
        for index, loader in enumerate(self.data_loaders):
            ...
            if self.fit_function == 'chi_squared':
                ...
                if self.atmmu_norm_error_fixed >= 0.:
                    ref_histo_w2 +=(loader.atmmu_histo['data'] + \
                                 (loader.atmmu_histo['data'] - loader.atmmu_histo['data_aux'])**2)*self.atmmu_norm_error_fixed**2
                else:
                    ref_histo_w2 +=(loader.atmmu_histo['data'] + \
                                 (loader.atmmu_histo['data'] - loader.atmmu_histo['data_aux'])**2)*norm_atmmu**2
    ...
    def __call__(self, data_histograms = [], 
                 ...
                 atmmu_norm_error_fixed = 0.384953457939,    #this is a scaling factor from Joao P.
                 ):
	...
	atmmu_norm_error_fixed - scaling factor.  For 'standard behavior' set to a negiative value and this parameter will 
        ...
        '''
        self.atmmu_norm_error_fixed = atmmu_norm_error_fixed
	...
	print starting_eval_dict
        ...
        nu_norms  = [nu_events*results['nu_frac1']/(np.sum(neutrinos[0])*norm_nu_secs),
		     ...
		     nu_events*results['nu_frac4']/(np.sum(neutrinos[3])*norm_nu_secs),
                     ...
                     nu_events*results['nu_frac9']/(np.sum(neutrinos[8])*norm_nu_secs),  #now the eight bins of nue
                     nu_events*results['nu_frac10']/(np.sum(neutrinos[9])*norm_nu_secs),
                     nu_events*results['nu_frac11']/(np.sum(neutrinos[10])*norm_nu_secs),
                     nu_events*results['nu_frac12']/(np.sum(neutrinos[11])*norm_nu_secs),
		     np.abs(nu_events*
                            (1-results['nu_frac1']-results['nu_frac2']-results['nu_frac3']-results['nu_frac4']-
                             ...
                             results['nu_frac9'] -results['nu_frac10']-results['nu_frac11']-results['nu_frac12'])/\
                                (np.sum(neutrinos[12])*norm_nu_secs))]

	print '                                                 '
	print 'results[nu_frac1]'
	print results['nu_frac1'], results['nu_frac1']
	print '  neutrinos[0] , neutrinos[1]                                                  '
	print neutrinos[0],neutrinos[0]
	print '                                                 '
